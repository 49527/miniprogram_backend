...
class BookkeepingView(WLAPIView, APIView):
    def post(self, request):
        data, context = self.get_request_obj(request)
        seri = BookkeepingSerializer(data=data)
        self.validate_serializer(seri)

        oid = bookkeeping(**seri.validated_data)
        return self.generate_response(
            data={"oid": oid},
            context=context
        )
...
class BookkeepingSanOrderView(WLAPIView, APIView):
    def post(self, request):
        data, context = self.get_request_obj(request)
        seri = BookkeepingSanOrderSerializer(data=data)
        self.validate_serializer(seri)
